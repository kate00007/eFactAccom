(function () {
    ...
    angular.module('app')
              .controller('mainController', ['$scope', '$http', '$location', '$route', 'FileSaver', 'Blob', 'dataFactory', 'Utils', 'ProductInfo', function ($scope, $http, $location, $route, FileSaver, Blob, dataFactory, Utils, ProductInfo) {
                  ...
                  $scope.GetLowResImage = function (a_options, a_filename) {
                      //console.log("GetLowRes" + a_filename);
                      if (a_filename) {
                          var retValue = false;
                  ...
                  $scope.init = function () {
                      ...
                      dataFactory.getProductInfo()
                        .success(function (data, status, headers, config) {
                            ...
                            console.log("My Data::: ---" + JSON.stringify($scope.myData));

                            console.log("Participants:" + JSON.stringify($scope.myData.participants));

                            $scope.productInfo = new ProductInfo($scope.myData);
                            //console.log("---Logo" + $scope.productInfo.getLogo());
                            //console.log("---getDegcoor" + JSON.stringify($scope.productInfo.getDegreeCoordinates()));
                            //console.log("---productDescription" + $scope.productInfo.getProductHomePageDescription());

                            // console.log("---Room List " + JSON.stringify($scope.productInfo.getRoomList()));
                            //set coordinates
                            ...
                            //classify product images : room, day span, food and beverage, fitness, additionals


                            if ($scope.myData.option.length > 0) {
                                //console.log("------Product Images -------");
                                //console.log($scope.myData.option.length);
                  ...
                  $scope.GetRatingsHeader = function (value) {
                      var header = "";
                      switch (value) {
                          case 'Ratings_Awards': header = 'Awards and Affiliations: Award Description'; break;
                          case 'Ratings_TransportAirport': header = 'Transport: Complimentary airport transfers included'; break;
                          case 'Ratings_TransportCity': header = 'Transport: City or local transfers provided by property'; break;
                          case 'Ratings_MealPlan': header = 'Meal Plan - Lead-in price inclusive of:'; break;
                          case 'Ratings_InsurancePublicLiability': header = 'Insurance: Public Liability'; break;
                          case 'Ratings_InsurancePersonal': header = 'Insurance: Personal Indemnity'; break;
                          case 'Ratings_MinStay': header = 'Minimum Stay: Minimum stay'; break;
                          case 'Ratings_ChildPolicyAdultsOnly': header = 'Child Policy: Adults Only'; break;
                          case 'Ratings_ChildPolicyChildFree': header = 'Child Policy: Children free when using existing bedding'; break;
                          case 'Ratings_Servicing': header = 'Servicing: Servicing'; break;
                          case 'Ratings_SpecialOffersStayPay': header = 'Special Offers: Stay pay offer'; break;
                          case 'Ratings_SpecialOffersUpgrade': header = 'Special Offers: Upgrade offer'; break;
                          case 'Ratings_SpecialOffersMeal': header = 'Special Offers: Meal offer'; break;
                          case 'Ratings_SpecialOfferAirportTransfer': header = 'Specials Offers: Airport transfer offer'; break;
                          case 'Ratings_SpecialOfferResortVoucher': header = 'Specials Offers: Resort voucher offer'; break;
                          case 'Ratings_SpecialOfferFlowers': header = 'Special Offers: Flowers or wine on arrival offer'; break;
                          case 'Ratings_SpecialOfferHoneymoon': header = 'Special Offers: Honeymoon offer'; break;
                          case 'Ratings_SpecialOfferOthers': header = 'Special Offers: Other offers including details and validity'; break;
                  ...
                  $scope.GetAgeRange = function (value) {
                      ...
                      if (value) {
                          var strArr = value.split("Age Range=");
                  ...
                  $scope.GetFlyingDistance = function (value) {
                      ...
                      if (value) {
                          ...
                          if ((strArr[1]).trim() !=== 'null') {
                              retVal = (strArr[1]).trim() || false;
                          } else {
                             retVal = false;
                          }
                  ...
                  $scope.GetGolfDetails = function (value) {
                      //Y Complimentary=N Course=null   console.log("Get Golf Details:" + value);
                      if (value) {
